...
OpenRTB は一般的に、取引所とそのデマンドソースの間のインタラクション、あるいは競売人とその入札者とのインタラクションとして考えられています。 その点において OpenRTB は、取引所からそのデマンドパートナーへ入札リクエストを公告し、レスポンスの中から入札を集め、オークションルールのいくつかの形式に基づいて勝者を決定することで、小商品（例：広告インプレッション）を販売する方法を定義します。 取引所は、買い手に彼らの入札が受け入れられたことを通知し、ビジネスポリシー（例：広告マークアップの描画）に基づいて入札が請求可能になったときに、取引所は清算価格といった重要なデータとともに勝利した買い手に通知します。 それ以外の買い手がオークションに負けたことを通知されることもあります。
<!--
...
* 私が受け取った全ての保留、請求、敗北イベントは、私の供給側パートナー（私のすぐ左の主体）から来ている、もしくはその完全な権限を有しているものとみなします。
...
異なる仕様間におけるオブジェクトの再利用を支援し、仕様の個々の側面がそれぞれのペースで発展できるように、OpenRTB v3.0 以降では階層化アプローチが採用されています。このモデルを次に図示します。形式ばらずに表現すれば、レイヤ１は当事者間でバイトデータを移動し、レイヤ２はこのバイトデータの言語を表現し、レイヤ３はこの言語を使った売買処理を指定し、レイヤ４は売買処理される商品について説明しています。
<!--
...
<table>
  ...
  <tr>
    ...
    <td>整数</td>
    <td>取引所が入札の受け付けに許容するミリ秒最大値で、タイムアウトを避けるためにインターネットレイテンシを含んでいる。この値は、取引所からの<em>事前の</em>ガイダンスに取って代わるものである。取引所が仲介者として振る舞う場合、レイテンシと追加のインターネットホップを考慮して、受け取った値よりも受け渡す <code>tmax</code> 値を減らすべきである。</td>
  </tr>
  <tr>
    <td><code>at</code></td>
...
このオブジェクトは、トランザクションの送信元についてのデータを保持していて、トランザクション自体のユニーク ID、送信元認証情報、CoC を含んでいます。
<!--
...
<table>
  ...
  <tr>
    ...
    <td>文字列</td>
    <td>入札に負けたことがわかったとき、取引所から呼び出される敗北通知 URL。置換マクロが含まれることがある。ただし、取引所固有のポリシーにより敗北通知のサポートや落札価格の開示が阻まれ、その結果 <code>${OPENRTB_PRICE}</code> マクロが削除（長さ 0 の文字列に置き換え）されることがある。</td>
  </tr>
  <tr>
    <td><code>exp</code></td>
    <td>整数</td>
