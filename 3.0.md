...
<table>
  ...
  <tr>
    ...
    <td>文字列</td>
    <td>入札に負けたことがわかったとき、取引所から呼び出される敗北通知 URL。置換マクロが含まれることがある。ただし、取引所固有のポリシーにより敗北通知のサポートや落札価格の開示が阻まれ、その結果 <code>${OPENRTB_PRICE}</code> マクロが削除（長さ 0 の文字列に置き換え）されることがある。
  </tr>
  <tr>
    <td><code>exp</code></td>
    <td>整数</td>
