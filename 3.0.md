...
| ------- | -------- |
| 概要（OVERVIEW） | DONE |
| アーキテクチャ（ARCHITECTURE） | DONE |
| 仕様（SPECIFICATION） | IN PROGRESS |
| 例（EXAMPLES） | TBD |
...
IAB テクノロジーラボラトリ（テックラボ）は、効果的かつ持続可能なグローバルデジタルメディアエコシステムの成長を促進するための標準、ソフトウェア、およびサービスを制作し、提供する非営利の研究開発団体です。 IAB テックラボは、デジタル配信社や広告技術会社、さらにマーケティング担当者、代理店、インタラクティブマーケティング分野に関心のある会社で構成され、モバイルおよびテレビ・デジタルビデオチャネルの有効化に重点を置くことで、透明で安全かつ効果的なサプライチェーン、より簡単でより一貫した測定、そして消費者にとってのより良い広告体験を通じて、ブランドとメディアの成長を実現することを目指しています。 IAB テックラボは、消費者、配信社、広告主、およびサードパーティプラットフォームのデジタル体験を向上させるために設計された DigiTrust  標準リアルタイムアイデンティティサービスも所有しています。ボードメンバーには AppNexus、ExtremeReach、Google、GroupM、Hearst Digital Media、Integral Ad Science、Index Exchange、LinkedIn、MediaMath、Microsoft、Moat、Pandora、PubMatic、Quantcast、Telaria、Yahoo! Japan が参加します。 IAB テックラボは2014年に設立されニューヨークに本社を置き、サンフランシスコに事務所、シアトルとロンドンに代理店を構えます。
...
IAB テックラボについて [www.iabtechlab.com](https://www.iabtechlab.com) でさらに知ることが出来ます。
<!--
...
OpenRTB は業界で非常に幅広く採用されているため、2012年1月にバージョン 2.1 のリリースに併せて IAB 標準として発行されました。仕様の技術的な内容に関する意思決定は、OpenRTB コミュニティとそのガバナンスルールに基づきます。
<!--
...
2.4 オーディオ広告帯のサポートと、2.x バージョンの中で最も多くの軽度から中程度の機能拡張。
<!--
...
<table>
  ...
  <tr>
    <td>RTB<!-- RTB --></td>
    <td>リアルタイムもしくはそれに近い個々の入札。<!-- Bidding for individual items in real-time or near real-time. --></td>
  </tr>
  <tr>
    <td>供給元<!-- Supply&nbsp;Source --></td>
    <td>配信者ないし在庫を売っているオーディエンスへの訴求元。<!-- The publisher or other audience source selling inventory. --></td>
  </tr>
  <tr>
    <td>取引所 <!-- exchange --></td>
    <td>個々の入札者間のオークションを執りしきる主体。（例：SSP）<!-- An entity (e.g., an SSP) that conducts an auction among bidders per item. --></td>
  </tr>
  <tr>
    <td>媒体 <!-- Intermediary --></td>
    <td>供給元と需要元の間にある主体。（例：取引所）<!-- An entity in the path between supply and demand sources (e.g., an exchange). --></td>
  </tr>
  <tr>
    <td>サプライチェーン<!-- Supply&nbsp;Chain --></td>
    <td>最後の供給元と需要元までの間にある複数の媒体のセット。<!-- A set of 1+ intermediaries between the ultimate supply and demand sources. --></td>
  </tr>
  <tr>
    <td>需要元 <!-- Demand&nbsp;Source --></td>
    <td>オークションへ個々に入札する主体。（例：DSP）<!-- An entity (e.g., a DSP) that bids for items into an auction. --></td>
  </tr>
  <tr>
    <td>シート <!-- Seat --></td>
    <td>個々の獲得を目指して、そのために需要プラットフォームを使う購入主体（例：広告主、代理店）。 通常は広告予算の所有者。 <!-- A buying entity (e.g., advertiser, agency) that wishes to obtain items and use a demand platform to act on its behalf; usually the owner of the buying budget. --></td>
  </tr>
  <tr>
    <td>取引 <!-- Deal --></td>
    <td>特定の条件の下での個々の購入についての売り手と買い手の間の協定。<!-- An agreement between sellers and buyers to purchase items under certain terms. --></td>
  </tr>
</table>
...
しかし、昨今のエコシステムははるかに複雑です。 ヘッダービディングが極めて一般的になり、配信者やユーザーエージェントもしくはその近くに新たな決定点がもたらされ、取引所をもはや唯一の決定者にならないようにしました。 また、複数のサプライチェーン媒体を持つことも一般的になりました。 それは直接統合された需要に加えて、別の取引所を需要元として使用する仲介者または取引所である可能性があります。
<!--
...
これらの構造は、次の参照モデル図で示されています。取引所は上流（向かって左）媒体への入札を行う需要元として振る舞ってもよいですし、他の下流（向かって右）取引所および DSP を需要元として使用してもよいですし、その両方が行われてもかまいません。ただし、図は *N* 個の取引所もしくは供給媒体の一般的なケースを示していますが、実務上の考慮事項（例：レイテンシ、連続収益の分配など）によって自然な範囲での制限が課されることがあります。
<!--
...
取引所とその需要元との間の基本プロトコルは HTTP です。 特に、入札リクエストを HTTP GET よりも大きなペイロードに対応し、かつバイナリ表現の使用を容易にするために HTTP POST が必要とされます。 通知イベントは、取引所の裁量で POST または GET のいずれでも構いません。
<!--
...
**ベストプラクティス：** 接続パフォーマンスを向上させる最も簡単で効果的な方法の1つは、Keep-Alive とも呼ばれる HTTP 永続接続を有効にすることです。 インタフェイスの両側での接続管理オーバーヘッドと CPU 使用率を削減することによって、全体的なパフォーマンスに大きな影響を与えます。
<!--
...
トランザクションレイヤは OpenRTB を含むリアルタイム入札プロトコルの心臓です。 取引所とその入札者との間、より一般的にはサプライチェーン媒体とその統合された需要元との間の通商プロトコルを定義しています。
<!--
...
完全な詳細を知りたければ、このドキュメントの「仕様」の章を参考にしてください。
<!--
