...
しかし、昨今のエコシステムははるかに複雑です。ヘッダービディングが極めて一般的になり、配信者やユーザーエージェントもしくはその近くに新たな決定点がもたらされ、取引所をもはや唯一の決定者にならないようにしました。 そして、複数のサプライチェーン仲介者を持つことも一般的になりました。仲介者は直接統合されたデマンドとは限らず、メディエータを始めとする別の取引所をデマンドソースとして使用する取引所である可能性があります。
<!--
