...
以下の用語は本書を通して、OpenRTB および本仕様の文脈において明確に使用されています。
<!--
...
OpenRTB は一般的に、取引所とそのデマンドソースとの間の相互作用、もしくは時として競売人とその入札者との相互作用として考えられています。 その点において OpenRTB は、取引所からその需要パートナーへ入札リクエストを公告し、レスポンスから入札を集め、オークションルールのいくつかの形式に基づいて勝者を決定することで、小商品（例：広告インプレッション）を販売する方法を定義します。 取引所は、買い手に彼らの入札が受け入れられたことを通知し、ビジネスポリシー（例：広告マークアップの描画）に基づいて入札が請求可能になったときに、取引所は清算価格といった重要なデータとともに勝利した買い手に通知します。 それ以外の買い手がオークションに負けたことを通知されることもあります。
<!--
...
仕様準拠の観点として、*必須* と示されていない全ての属性は、*推奨* か否かに関わらず任意になります。任意の属性は、省略された場合に推定されるデフォルト値を持っていても構いません。もしデフォルト値がないことを示されていれば、慣習的に *不明* として解釈されるべきです。また、空の文字列やヌル値は、省略されたものと同じと解釈されるべきです。（指定されていればデフォルト値、そうでなければ *不明*）
<!--
